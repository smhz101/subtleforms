...
/**
 * FormPreviewPane Component
 *
 * Renders a read-only preview of the current draft schema as an inline pane
 * (no modal wrapper). Reuses FormPreviewModal.scss for consistent styling.
 *
 * ⚠️ Same as FormPreviewModal but without the Modal chrome — intended for
...
export default function FormPreviewPane({ schema, isDirty = false }) {
  ...
  const renderField = (field, index) => {
    ...
    switch (field.type) {
        ...
        return (
          <div key={field.key || index} className='sf-form-preview-field'>
            ...
            <select
              ...
              {(config.options || field.options)?.map((opt, i) => (
      ...
      case 'checkbox':
        if (!(config.options || field.options)?.length) {
          return (
            <div key={field.key || index} className='sf-form-preview-field'>
              ...
              <div className='sf-captcha-preview__content'>
                ...
                {config.providerName && (
                  <div className='sf-captcha-preview__provider'>
                    {config.providerName === 'recaptcha' &&
                      'Google reCAPTCHA'}
                    {config.providerName === 'hcaptcha' && 'hCaptcha'}
                    {config.providerName === 'turnstile' &&
  ...
  return (
    <div className='sf-form-preview-pane'>
      {/* Form Header */}
